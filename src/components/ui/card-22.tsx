...
export function PlaceCard({
  ...
  return (
    <motion.div
      ...
      className={cn('overflow-hidden border border-ink/10 bg-white/58 p-3 shadow-editorial backdrop-blur-xl', className)}
    >
      <div className="group relative h-72 overflow-hidden">
        ...
        <div className="absolute right-3 top-3 border border-white/50 bg-white/70 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.18em] text-ink backdrop-blur-md">
          In Curation
        </div>
        <div className="absolute inset-x-3 top-1/2 flex -translate-y-1/2 justify-between opacity-0 transition-opacity group-hover:opacity-100">
          <button className="grid h-10 w-10 place-items-center bg-ink/45 text-white backdrop-blur" onClick={() => changeImage(-1)} aria-label="Previous image">
            <ChevronLeft className="h-5 w-5" />
          </button>
          <button className="grid h-10 w-10 place-items-center bg-ink/45 text-white backdrop-blur" onClick={() => changeImage(1)} aria-label="Next image">
